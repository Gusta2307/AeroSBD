...
def employee_installation_purchases_callback_query(update, context):
    ...
    if query == "cant":
        ...
        if context.user_data['codigo_prod'] == "" or context.user_data['cant_prod'] == "":
            ...
            return EMPLOYEE_INSTALLATION_PURCHASES
        cant_exits = select_cant_prod(select_ID_A_employee_using_id_telegram(update.effective_user['id']), select_id_installation_employee(update.effective_user['id']), context.user_data['codigo_prod'])[0]
        print(cant_exits)
